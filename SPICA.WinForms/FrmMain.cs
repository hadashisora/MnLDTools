...
namespace SPICA.WinForms
{
    public partial class FrmMain : Form
    {
        ...
        private void FileOpenFromByteArray(byte[] data, bool MergeMode)
        {
            if (!MergeMode)
            {
                Renderer.DeleteAll();
                ...
                Scene = FileIO.MergeFromBytearray(data, Renderer);
        ...
        public void OpenFromBytearray(byte[] input)
        {
            IgnoreClicks = true;

            FileOpenFromByteArray(input, false);
        ...
        private void DumpRenderer()
        {
            using (FolderBrowserDialog FolderDlg = new FolderBrowserDialog())
            {
                if (FolderDlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        string FileName = Path.Combine(FolderDlg.SelectedPath, $"{Texture.Key}.png");
